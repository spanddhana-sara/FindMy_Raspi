...
//      print(result)
      ...
      var decodedBytes = [UInt8]()
        ...
        print(decodedBytes) // decodedBytes.reversed()
       // print(decodedBytes.reverse())
        let reverseStr = String(decodedStr.reversed())
        print(reverseStr)
        logAndPrint("Result: Message \(messageID) bitstring: \(reverseStr) bytestring: \(decodedBytes)" , fileHandle: logger)
      message?.decodedBytes = decodedBytes
      message?.decodedStr = decodedStr
      message?.reversedStr = reverseStr
        
        print("Result bytestring: \(reverseStr)")
      ...
      // Not finished yet -> Next round
        if (result.keys.max()! + 1 < 16){
            print("Haven't found end byte yet. Starting with byte \(result.keys.max()! + 1) now")
